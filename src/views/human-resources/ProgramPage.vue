...
import { ProgramResponse } from '@/typings/models.types.ts'
...
import { useLibrariesStore, ProgramPayload } from '@/stores/libraries.store'
import { formatDate } from '@/utils/helpers'

const programStore = useLibrariesStore()
...
onBeforeMount(async () => {
  programIsLoading.value = true
  const response = await programStore.fetchListPrograms(paginationLimit)
  if (response.success && response.pagination) {
    pagination.value = response.pagination
    console.log('Fetched Program rows:', response.data)
  }
  programIsLoading.value = false
})

const handlePaginationPageChange = async (event: PageState) => {
  const pageSelected = event.page + 1
  programIsLoading.value = true
  const response = await programStore.fetchListPrograms(paginationLimit, pageSelected)
  if (response.success && response.pagination) {
    pagination.value = response.pagination
  }
  programIsLoading.value = false
...
const handleSearchProgram = async () => {
  ...
  const response = await programStore.searchListPrograms(searchQuery.value)
  if (response.success && response.pagination) {
    pagination.value = response.pagination
...
const props = defineProps<programDetailsFormProps>()
onMounted(async () => {
...
const validator = useVuelidate<Partial<ProgramPayload>>(formRules, payload)
const handleSaveSubmissionif = async () => {
  ...
  try {
    ...
    toast.add({
      severity: 'success',
      summary: 'Success',
      detail: 'Program submitted successfully',
      life: 5000,
    })
  ...
  <div class="flex h-full w-full flex-col shadow-md">
    <div class="h-full w-full rounded-md bg-surface-0 p-6">
      ...
      >
        ...
        <div class="flex w-full items-center justify-end gap-4">
          <div class="gap-4 whitespace-nowrap md:w-auto">
            <Button
              icon="pi pi-plus"
              v-tooltip.top="'Program Creation'"
              severity="info"
              size="large"
          ...
          <div class="flex w-full md:w-auto lg:w-1/2">
            <InputGroup v-model="searchQuery" class="w-full">
              <InputText
                v-model="searchQuery"
                placeholder="Search via Program Name"
                class="w-full"
                :disabled="programIsLoading"
                @keyup.enter="handleSearchProgram"
              />
              <Button icon="pi pi-search" @click="handleSearchProgram" :loading="programIsLoading" :disabled="programIsLoading" />
            </InputGroup>
          </div>
        </div>
      </div>
      ...
      <div class="mt-6 flex flex-col">
        ...
        <div
          v-if="searchSubmitted && !programIsLoading && !programStore.programs.length"
