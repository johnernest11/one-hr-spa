...
import { DivisionResponse } from '@/typings/models.types.ts'
...
import { useLibrariesStore, DivisionPayload } from '@/stores/libraries.store'
...
const CreateDivision = ref(false)
...
const openDivisionDialog = (division: DivisionResponse | null = null) => {
  ...
  if (division) {
    updatePayloadFromReport(division)
    isEditMode.value = true
  } else {
    resetPayload() // clear form if new
    isEditMode.value = false
  }
  CreateDivision.value = true
}

const opendivisionForm = () => {
  resetPayload()
  CreateDivision.value = true
}

const payload = reactive<DivisionPayload>({
  name: '',
})
...
onBeforeMount(async () => {
  ...
  const response = await divisionStore.fetchListDivisions(paginationLimit)
  if (response.success && response.pagination) {
    pagination.value = response.pagination
    console.log('Fetched Division rows:', response.data)
  }
...
const handlePaginationPageChange = async (event: PageState) => {
  ...
  divisionIsLoading.value = true
  const response = await divisionStore.fetchListDivisions(paginationLimit, pageSelected)
  if (response.success && response.pagination) {
    pagination.value = response.pagination
  }
  divisionIsLoading.value = false
...
const handleSearchDivision = async () => {
  divisionIsLoading.value = true
  ...
  const response = await divisionStore.searchListDivisions(searchQuery.value)
  if (response.success && response.pagination) {
    pagination.value = response.pagination
...
type divisionDetailsFormProps = {
  division?: DivisionResponse
}
const props = defineProps<divisionDetailsFormProps>()
onMounted(async () => {
  const id = route.params.id as string
  if (id) {
    const response = await divisionStore.fetchListDivisions()
    if (response && response.success) {
      updatePayloadFromReport(response.data as DivisionResponse)
    }
  }
  isLoading.value = false
})

const updatePayloadFromReport = (division: DivisionResponse | null) => {
  payload.name = division?.name ?? ''
}
...
const validator = useVuelidate<Partial<DivisionPayload>>(formRules, payload)
const handleSaveSubmissionif = async () => {
  ...
  try {
    ...
    toast.add({
      severity: 'success',
      summary: 'Success',
      detail: 'Division submitted successfully',
      life: 5000,
    })
...
<template>
  <div class="flex h-full w-full flex-col shadow-md">
    <div class="h-full w-full rounded-md bg-surface-0 p-6">
      ...
      >
        ...
        <div class="flex w-full items-center justify-end gap-4">
          <div class="gap-4 whitespace-nowrap md:w-auto">
            <Button
              icon="pi pi-plus"
              v-tooltip.top="'Division Creation'"
              severity="info"
              size="large"
          ...
          <div class="flex w-full md:w-auto lg:w-1/2">
            <InputGroup v-model="searchQuery" class="w-full">
              <InputText
                v-model="searchQuery"
                placeholder="Search via Division Name"
                class="w-full"
                :disabled="divisionIsLoading"
                @keyup.enter="handleSearchDivision"
              />
              <Button
                icon="pi pi-search"
                @click="handleSearchDivision"
      ...
      <div class="mt-6 flex flex-col">
        ...
        <div
          v-if="searchSubmitted && !divisionIsLoading && !divisionStore.divisions.length"
  ...
  <!-- Create/Update DIvision Dialog -->
  <Dialog v-model:visible="CreateDivision" modal header="Division Creation" :style="{ width: '90vw' }">
    <template #header>
      <div class="flex items-center space-x-3 pt-4 sm:px-6 md:px-8">
        <h1 class="font-base text-2xl text-surface-600 sm:text-xl md:text-2xl">Division Creation</h1>
      </div>
    </template>
    <hr />
